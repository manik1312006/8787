...
fetch('data.csv')
    ...
    .then(data => {
        ...
        movies = rows.map(row => {
            const [name, url, image, date, genre] = row.split(',').map(item => item.replace(/"/g, '').trim());
            const genres = genre ? genre.split(',').map(g => g.trim()) : []; // Handle cases where genre might be empty
            genres.forEach(g => allGenres.add(g));
            return { name, url, image, date, genres };
        });
